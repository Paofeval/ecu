...
Regions are the key spatial units of the Ocean Health Index. Scores are calculated for each region individually and combined (with an offshore area-weighted average) to produce scores for the entire study area: {{ site.study_area }}. The regions below can be modified. These template regions were identified as the largest subcountry division within {{ site.study_area }}.
